...
dataSet = 1; %Rodar de 1 a 6
parametersSet = 7; %Não modificar

results = zeros(numberOfExperiments,4);
resultsConvergence = zeros(numberOfExperiments,1);
...
switch(dataSet)
  case 1
    ...
    convergenceIUCFileName=strcat('IUC_',fileName);
...
for ac=1:numberOfExperiments
  ...
  main_mmwalbp
  results(ac,:)=melhores;
  resultsConvergence(ac,:)= convergence;
end
...
fidConvIUC = fopen(convergenceIUCFileName , 'w');

for i=1:size(resultsConvergence,1)
  
  fprintf(fidConvIUC, '%f ' , resultsConvergence(i,:));
  fprintf(fidConvIUC, '\r\n');
